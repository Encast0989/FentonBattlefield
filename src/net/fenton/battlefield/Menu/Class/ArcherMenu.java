 ...
 */
public class ArcherMenu extends FentonInventory {

    public ArcherMenu() {
        super("Archer", false, 45);
    }

    @Override
    public void openInventory(Player player) {
        Inventory i = getInventory();
        ClassMenu.getInstance().openInventory(player, i, BattlefieldClassData.BFClass.ARCHER);
    }
    ...
    public void onClick(Player p, ItemStack i) {
        if(i.hasItemMeta()) {
            ...
            } else if(name.equalsIgnoreCase("Archer UPGRADE")) {
                BFPlayer bfp = BFPlayerData.getInstance().getPlayers().get(p.getUniqueId());
                if(bfp.getClassLevel().getLevelByName(BattlefieldClassData.BFClass.ARCHER) < 5) {
                    ClassMenu.getInstance().upgradeClass(p, BattlefieldClassData.BFClass.ARCHER,
                            (bfp.getClassLevel().getLevelByName(BattlefieldClassData.BFClass.ARCHER) + 1));
                } else {
                    p.sendMessage("§cYou already have the max level for this class!");
                }
            } else if(name.equalsIgnoreCase("Ability UPGRADE")) {
                BFPlayer bfp = BFPlayerData.getInstance().getPlayers().get(p.getUniqueId());
                if(bfp.getClassLevel().getAbilityLevelByName(BattlefieldClassData.BFClass.ARCHER) < 5) {
                    ClassMenu.getInstance().upgradeAbility(p, BattlefieldClassData.BFClass.ARCHER,
                            (bfp.getClassLevel().getAbilityLevelByName(BattlefieldClassData.BFClass.ARCHER) + 1));
                } else {
                    p.sendMessage("§cYou already have the max ability level for this class!");
                }
            } else if(name.equalsIgnoreCase("Choose Class")) {
                ClassMenu.getInstance().chooseClass(p, BattlefieldClassData.BFClass.ARCHER);
            }
        }
    }
}
