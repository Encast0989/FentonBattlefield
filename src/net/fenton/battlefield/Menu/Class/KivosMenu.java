 ...
 */
public class KivosMenu extends FentonInventory {

    public KivosMenu() {
        super("Kivos", false, 45);
    }

    @Override
    public void openInventory(Player player) {
        Inventory i = getInventory();
        ClassMenu.getInstance().openInventory(player, i, BattlefieldClassData.BFClass.KIVOS);
    }
    ...
    public void onClick(Player p, ItemStack i) {
        if(i.hasItemMeta()) {
            ...
            } else if(name.equalsIgnoreCase("Kivos UPGRADE")) {
                BFPlayer bfp = BFPlayerData.getInstance().getPlayers().get(p.getUniqueId());
                if(bfp.getClassLevel().getLevelByName(BattlefieldClassData.BFClass.KIVOS) < 5) {
                    ClassMenu.getInstance().upgradeClass(p, BattlefieldClassData.BFClass.KIVOS,
                            (bfp.getClassLevel().getLevelByName(BattlefieldClassData.BFClass.KIVOS) + 1));
                } else {
                    p.sendMessage("§cYou already have the max level for this class!");
                }
            } else if(name.equalsIgnoreCase("Ability UPGRADE")) {
                BFPlayer bfp = BFPlayerData.getInstance().getPlayers().get(p.getUniqueId());
                if(bfp.getClassLevel().getAbilityLevelByName(BattlefieldClassData.BFClass.KIVOS) < 5) {
                    ClassMenu.getInstance().upgradeAbility(p, BattlefieldClassData.BFClass.KIVOS,
                            (bfp.getClassLevel().getAbilityLevelByName(BattlefieldClassData.BFClass.KIVOS) + 1));
                } else {
                    p.sendMessage("§cYou already have the max ability level for this class!");
                }
            } else if(name.equalsIgnoreCase("Kivos")) {
                p.sendMessage("§cYou already have the max level for this class!");
            }  else if(name.equalsIgnoreCase("Choose Class")) {
                ClassMenu.getInstance().chooseClass(p, BattlefieldClassData.BFClass.KIVOS);
            }
        }
    }
}
