 ...
 */
public class BlastyMenu extends FentonInventory {

    public BlastyMenu() {
        super("Blasty", false, 45);
    }

    @Override
    public void openInventory(Player player) {
        Inventory i = getInventory();
        ClassMenu.getInstance().openInventory(player, i, BattlefieldClassData.BFClass.BLASTY);
    }
    ...
    public void onClick(Player p, ItemStack i) {
        if(i.hasItemMeta()) {
            ...
            } else if(name.equalsIgnoreCase("Blasty UPGRADE")) {
                BFPlayer bfp = BFPlayerData.getInstance().getPlayers().get(p.getUniqueId());
                if(bfp.getClassLevel().getLevelByName(BattlefieldClassData.BFClass.BLASTY) < 5) {
                    ClassMenu.getInstance().upgradeClass(p, BattlefieldClassData.BFClass.BLASTY,
                            (bfp.getClassLevel().getLevelByName(BattlefieldClassData.BFClass.BLASTY) + 1));
                } else {
                    p.sendMessage("§cYou already have the max level for this class!");
                }
            } else if(name.equalsIgnoreCase("Ability UPGRADE")) {
                BFPlayer bfp = BFPlayerData.getInstance().getPlayers().get(p.getUniqueId());
                if(bfp.getClassLevel().getAbilityLevelByName(BattlefieldClassData.BFClass.BLASTY) < 5) {
                    ClassMenu.getInstance().upgradeAbility(p, BattlefieldClassData.BFClass.BLASTY,
                            (bfp.getClassLevel().getAbilityLevelByName(BattlefieldClassData.BFClass.BLASTY) + 1));
                } else {
                    p.sendMessage("§cYou already have the max ability level for this class!");
                }
            } else if(name.equalsIgnoreCase("Choose bfclass")) {
                ClassMenu.getInstance().chooseClass(p, BattlefieldClassData.BFClass.BLASTY);
            }
        }
    }
}
